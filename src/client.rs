...
use crate::transport::{
    auth::Auth,
    crypto,
    http::{request, Method, Url, HeaderMap},
    resolver::Resolver,
};
...
impl Client<'_> {
    ...
    pub fn put (&mut self, user_id: &str, repo_name: &str, path: &str, payload: &str) -> Result<Url, String> {
        let url = &self
            .homeservers_cache
            .get(user_id)
            .unwrap()
            ...
            .clone()
            .unwrap()
            .join(&format!("/mvp/users/{}/repos/{}/{}", user_id, repo_name, path))
            .unwrap();
        ...
        headers.insert("Content-Length", payload.len().to_string().try_into().unwrap());
...
mod tests {
    ...
    fn test_client_put() {
        ...
        assert_eq!(result.unwrap(), Url::parse(&server.url()).unwrap().join(&path).unwrap());
        assert_eq!(client.homeservers_cache.len(), 1);
        assert_eq!(
            client
                .homeservers_cache
